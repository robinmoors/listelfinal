 ...
 * @package entity
 */
class PsyCrisis extends Db2PhpEntityBase implements Db2PhpEntityModificationTracking {
	private static $CLASS_NAME='PsyCrisis';
	const SQL_IDENTIFIER_QUOTE='`';
	const SQL_TABLE_NAME='psy_crisis';
	const SQL_INSERT='INSERT INTO `psy_crisis` (`id`,`overleg_id`,`crisis_id`,`crisis_genre`,`crisissituatie`) VALUES (?,?,?,?,?)';
	const SQL_INSERT_AUTOINCREMENT='INSERT INTO `psy_crisis` (`overleg_id`,`crisis_id`,`crisis_genre`,`crisissituatie`) VALUES (?,?,?,?)';
	const SQL_UPDATE='UPDATE `psy_crisis` SET `id`=?,`overleg_id`=?,`crisis_id`=?,`crisis_genre`=?,`crisissituatie`=? WHERE `id`=?';
	const SQL_SELECT_PK='SELECT * FROM `psy_crisis` WHERE `id`=?';
	const SQL_DELETE_PK='DELETE FROM `psy_crisis` WHERE `id`=?';
	const FIELD_ID=-354499587;
	const FIELD_OVERLEG_ID=1140715426;
	const FIELD_CRISIS_ID=-1619523339;
	const FIELD_CRISIS_GENRE=-1853864087;
	const FIELD_CRISISSITUATIE=-740038133;
	 ...
	 * Query by Example.
	 *
	 * Match by attributes of passed example instance and return matched rows as an array of PsyCrisis instances
	 *
	 * @param PDO $db a PDO Database instance
	 * @param PsyCrisis $example an example instance defining the conditions. All non-null properties will be considered a constraint, null values will be ignored.
	 * @param boolean $and true if conditions should be and'ed, false if they should be or'ed
	 * @param array $sort array of DSC instances
	 * @return PsyCrisis[]
	 */
	public static function findByExample(PDO $db,PsyCrisis $example, $and=true, $sort=null) {
	 ...
	 *
	 * Will return matched rows as an array of PsyCrisis instances.
	 *
	 * @param PDO $db a PDO Database instance
	...
	/**
	 * Will execute the passed statement and return the result as an array of PsyCrisis instances
	 *
	...
	/**
	 * returns the result as an array of PsyCrisis instances without executing the passed statement
	 *
	 * @param PDOStatement $stmt
	 * @return PsyCrisis[]
	 */
	...
	/**
	 * Execute select query and return matched rows as an array of PsyCrisis instances.
	 *
	...
	protected function bindValues(PDOStatement &$stmt) {
		...
		$stmt->bindValue(3,$this->getCrisisId());
		$stmt->bindValue(4,$this->getCrisisGenre());
		$stmt->bindValue(5,$this->getCrisissituatie());
	}
	...
	public function insertIntoDatabase(PDO $db) {
		if (null===$this->getId()) {
			...
			$stmt->bindValue(2,$this->getCrisisId());
			$stmt->bindValue(3,$this->getCrisisGenre());
			$stmt->bindValue(4,$this->getCrisissituatie());
		} else {
	...
	/**
	 * Fetch Overlegbasis which references this PsyCrisis. Will return null in case reference is invalid.
	 * `psy_crisis`.`overleg_id` -> `overlegbasis`.`id`
	 *
	 * @param PDO $db a PDO Database instance
	 * @param array $sort array of DSC instances
	 * @return Overlegbasis
	...
	public function toDOM() {
		return self::hashToDomDocument($this->toHash(), 'PsyCrisis');
	}

	/**
	 * get single PsyCrisis instance from a DOMElement
	 *
	...
	/**
	 * get all instances of PsyCrisis from the passed DOMDocument
	 *
	...
	public static function fromDOMDocument(DOMDocument $doc) {
		$instances=array();
		foreach ($doc->getElementsByTagName('PsyCrisis') as $node) {
